...
| 缓存 | 路径 | 作用 |
|------|------|------|
| staging_dir | `immortalwrt/staging_dir` | 缓存 host 工具与交叉工具链（tools/toolchain 产物），命中后跳过这两阶段编译（实测约 36 分钟） |
| ccache | `immortalwrt/.ccache` | 缓存 C/C++ 编译产物，同版本重复编译大幅提速 |
| dl | `immortalwrt/dl` | 缓存源码包，避免重复下载，降低上游下载失败风险 |

- staging_dir 按 `系统-平台-版本` 分键且**不跨版本复用**（工具链与 tag 强绑定），不设 `restore-keys`
- ccache 按 `系统-平台-版本-compiler_check 版本`（当前末尾为 `-cc2`）分键，同平台同版本精确命中；跨版本保留 `restore-keys`，因为 ccache 用源码与命令行哈希校验，版本不符只会失效、不会用错
- `actions/cache` 在**主键完全命中时不会再保存缓存**：改动 `[3.5.1]` 的校验方式后，旧键恢复出来的条目已全部失效、新条目又写不进去，缓存会永远停在旧格式。因此每次改动校验方式都必须同步递增主键后缀（`cc3`、`cc4`…）
- dl 按 `系统-平台-版本` 分键且**不跨版本复用**：dl 里是未校验的压缩包，跨版本混用可能导致某个包源码与目标版本不匹配而报校验失败。代价是每个版本各存一份
- 三个缓存都带平台维度，将来新增不同平台的设备时不会互相污染
- GitHub 单个缓存上限 10GB，dl 与 staging_dir 通常在数 GB 级；若提示超限，编译不受影响，仅本次不保存对应缓存。`[4.3]` 会打印各构建目录的精确体积，用于评估后续的增量缓存策略
